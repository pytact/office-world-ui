// Project Pagination Hook
// Encapsulates pagination state management for project lists
// Following R5 rules: Business logic in hooks

import { useState, useCallback, useMemo } from "react";

interface ProjectPaginationMeta {
  total: number;
  page: number;
  page_size: number;
  total_pages: number;
...
interface UseProjectPaginationParams {
  initialPage?: number;
...
interface UseProjectPaginationReturn {
  ...
  updateFromResponse: (meta: ProjectPaginationMeta) => void;
  totalPages: number;
  hasNextPage: boolean;
...
/**
 * Hook for managing project list pagination state
...
export function useProjectPagination(
  ...
  const updateFromResponse = useCallback((meta: ProjectPaginationMeta) => {
