// Employee Pagination Hook
// Encapsulates pagination state management for employee lists
// Following R5 rules: Business logic in hooks

import { useState, useCallback, useMemo } from "react";

interface EmployeePaginationMeta {
  total: number;
  page: number;
  page_size: number;
  total_pages: number;
...
interface UseEmployeePaginationParams {
  initialPage?: number;
  initialPageSize?: number;
}

interface UseEmployeePaginationReturn {
  page: number;
  pageSize: number;
  ...
  updateFromResponse: (meta: EmployeePaginationMeta) => void;
  totalPages: number;
  hasNextPage: boolean;
...
/**
 * Hook for managing employee list pagination state
...
export function useEmployeePagination(
  ...
  const updateFromResponse = useCallback((meta: EmployeePaginationMeta) => {
