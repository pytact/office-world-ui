// Company Pagination Hook
// Encapsulates pagination state management and navigation
// Following R5 rules: Business logic in hooks

import { useState, useCallback, useMemo } from "react";
import { CompanyListPaginationMeta } from "@/utils/types/responses/company";

interface UseCompanyPaginationParams {
  initialPage?: number;
...
interface UseCompanyPaginationReturn {
  ...
  updateFromResponse: (meta: CompanyListPaginationMeta) => void;
  totalPages: number;
  hasNextPage: boolean;
...
/**
 * Hook for managing company list pagination state
...
export function useCompanyPagination(
  ...
  const updateFromResponse = useCallback(
    (meta: CompanyListPaginationMeta) => {
