// Leave Pagination Hook
// Encapsulates pagination state management for leave lists
...
interface UseLeavePaginationParams {
  initialPage?: number;
  initialPageSize?: number;
}

interface UseLeavePaginationReturn {
  page: number;
  pageSize: number;
  ...
  updateFromResponse: (meta: LeavePaginationMeta) => void;
  totalPages: number;
  hasNextPage: boolean;
...
/**
 * Hook for managing leave list pagination state
 ...
 */
export function useLeavePagination(
  params?: UseLeavePaginationParams
): UseLeavePaginationReturn {
  const [page, setPage] = useState(params?.initialPage || 1);
  const [pageSize, setPageSize] = useState(params?.initialPageSize || 20);
  const [paginationMeta, setPaginationMeta] =
    useState<LeavePaginationMeta | null>(null);
  ...
  const updateFromResponse = useCallback((meta: LeavePaginationMeta) => {
